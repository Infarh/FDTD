...
internal class Solver2DTest
{
    ...
    {
        const double k = 6.0;        // ширина гауссиан
        const double grn0 = 1.0 / 3; // сдвиг пика зелёного
        const double blu0 = 2.0 / 3; // сдвиг пика синего

        // защита от вырожденного диапазона
        if (dbMin == dbMax)
            return (255, 255, 255);

        // нормализация в [0..1] (dbMax -> 0, dbMin -> 1)
        var t = (dbValue - dbMax) / (dbMin - dbMax);

        // насыщение за пределами диапазона
        if (t < 0) t = 0;
        if (t > 1) t = 1;

        // гауссова «палитра»: красный пиковый у t=0, зелёный ~1/3, синий ~2/3
        static double GaussPeak(double x) => Math.Exp(-(x * x));

        var red = GaussPeak(t * k);
        var grn = GaussPeak((t - grn0) * k);
        var blu = GaussPeak((t - blu0) * k);
